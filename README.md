...
For each store in the input array, takes all the AmountEntries per TimePeriod and add their values together. The total for the TimePeriod is then added to a running total. The total is put into a new each AmountEntry and is placed in a new TemporalItemStore for the same TimePeriod. That new TemporalItemStore is stored in the same index that the input came from.
